...
CREATE TABLE LehrerNotenmodulCredentials (
  Lehrer_ID bigint NOT NULL, 
  Initialkennwort varchar(255) NOT NULL, 
  PasswordHash varchar(255) NOT NULL,
  CONSTRAINT PK_LehrerNotenmodulCredentials PRIMARY KEY (Lehrer_ID),
  CONSTRAINT LehrerNotenmodulCredentials_Lehrer_FK FOREIGN KEY (Lehrer_ID) REFERENCES K_Lehrer(ID) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE DavRessourceCollections (
  ...
  SyncToken datetime DEFAULT CURRENT_TIME NOT NULL, 
...
CREATE TABLE DavRessources (
  ...
  lastModified datetime DEFAULT CURRENT_TIME NOT NULL, 
  KalenderTyp varchar(120) NOT NULL, 
  KalenderStart datetime DEFAULT CURRENT_TIME NOT NULL, 
  KalenderEnde datetime DEFAULT CURRENT_TIME NOT NULL, 
  ressource longblob NOT NULL, 
  geloeschtam datetime,
...
CREATE TABLE EnmLeistungsdaten (
  ID bigint NOT NULL, 
  tsNotenKrz datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsNotenKrzQuartal datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsFehlStd datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsuFehlStd datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsLernentw datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsWarnung datetime DEFAULT CURRENT_TIME NOT NULL,
...
CREATE TABLE EnmLernabschnittsdaten (
  ID bigint NOT NULL, 
  tsSumFehlStd datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsSumFehlStdU datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsZeugnisBem datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsASV datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsAUE datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsBemerkungVersetzung datetime DEFAULT CURRENT_TIME NOT NULL,
...
CREATE TABLE EnmTeilleistungen (
  ID bigint NOT NULL, 
  tsDatum datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsLehrer_ID datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsArt_ID datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsBemerkung datetime DEFAULT CURRENT_TIME NOT NULL, 
  tsNotenKrz datetime DEFAULT CURRENT_TIME NOT NULL,
...
CREATE TABLE Schulbewerbung_Importe (
  Schueler_ID bigint NOT NULL, 
  LastSync datetime DEFAULT CURRENT_TIME NOT NULL, 
...
CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Vorgaben_1 AFTER INSERT ON Gost_Klausuren_Vorgaben FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Vorgaben';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Vorgaben_2 AFTER INSERT ON Gost_Klausuren_Vorgaben FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Vorgaben SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Vorgaben';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Vorgaben_3 AFTER INSERT ON Gost_Klausuren_Vorgaben FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Vorgaben), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Vorgaben', coalesce((SELECT max(ID) FROM Gost_Klausuren_Vorgaben), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Vorgaben_4 AFTER INSERT ON Gost_Klausuren_Vorgaben FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Vorgaben), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Vorgaben',  NEW.ID);
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Vorgaben_5 AFTER INSERT ON Gost_Klausuren_Vorgaben FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') IS NULL
BEGIN
  UPDATE Gost_Klausuren_Vorgaben SET ID = coalesce((SELECT max(ID) FROM Gost_Klausuren_Vorgaben), 0) + 1 WHERE ID = NEW.ID;
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Vorgaben',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Vorgaben), 0) + 1);
END;


CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Vorgaben_1 AFTER UPDATE ON Gost_Klausuren_Vorgaben FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Vorgaben';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Vorgaben_2 AFTER UPDATE ON Gost_Klausuren_Vorgaben FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Vorgaben SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Vorgaben';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Vorgaben_3 AFTER UPDATE ON Gost_Klausuren_Vorgaben FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Vorgaben), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Vorgaben', coalesce((SELECT max(ID) FROM Gost_Klausuren_Vorgaben), 0));
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Vorgaben_4 AFTER UPDATE ON Gost_Klausuren_Vorgaben FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Vorgaben), 0)
BEGIN
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Vorgaben_5 AFTER UPDATE ON Gost_Klausuren_Vorgaben FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Vorgaben') IS NULL
BEGIN
  -- Update der ID in der Tabelle Gost_Klausuren_Vorgaben erfolgt durch den Autoinkrement-Trigger 2, daher hier auch kein +1, sondern nur den Max-Wert schreiben
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Vorgaben',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Vorgaben), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Termine_1 AFTER INSERT ON Gost_Klausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Termine';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Termine_2 AFTER INSERT ON Gost_Klausuren_Termine FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Termine SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Termine';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Termine_3 AFTER INSERT ON Gost_Klausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Termine), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Termine', coalesce((SELECT max(ID) FROM Gost_Klausuren_Termine), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Termine_4 AFTER INSERT ON Gost_Klausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Termine), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Termine',  NEW.ID);
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Termine_5 AFTER INSERT ON Gost_Klausuren_Termine FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') IS NULL
BEGIN
  UPDATE Gost_Klausuren_Termine SET ID = coalesce((SELECT max(ID) FROM Gost_Klausuren_Termine), 0) + 1 WHERE ID = NEW.ID;
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Termine',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Termine), 0) + 1);
END;


CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Termine_1 AFTER UPDATE ON Gost_Klausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Termine';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Termine_2 AFTER UPDATE ON Gost_Klausuren_Termine FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Termine SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Termine';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Termine_3 AFTER UPDATE ON Gost_Klausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Termine), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Termine', coalesce((SELECT max(ID) FROM Gost_Klausuren_Termine), 0));
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Termine_4 AFTER UPDATE ON Gost_Klausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Termine), 0)
BEGIN
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Termine_5 AFTER UPDATE ON Gost_Klausuren_Termine FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Termine') IS NULL
BEGIN
  -- Update der ID in der Tabelle Gost_Klausuren_Termine erfolgt durch den Autoinkrement-Trigger 2, daher hier auch kein +1, sondern nur den Max-Wert schreiben
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Termine',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Termine), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Kursklausuren_1 AFTER INSERT ON Gost_Klausuren_Kursklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Kursklausuren';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Kursklausuren_2 AFTER INSERT ON Gost_Klausuren_Kursklausuren FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Kursklausuren SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Kursklausuren';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Kursklausuren_3 AFTER INSERT ON Gost_Klausuren_Kursklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Kursklausuren), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Kursklausuren', coalesce((SELECT max(ID) FROM Gost_Klausuren_Kursklausuren), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Kursklausuren_4 AFTER INSERT ON Gost_Klausuren_Kursklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Kursklausuren), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Kursklausuren',  NEW.ID);
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Kursklausuren_5 AFTER INSERT ON Gost_Klausuren_Kursklausuren FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') IS NULL
BEGIN
  UPDATE Gost_Klausuren_Kursklausuren SET ID = coalesce((SELECT max(ID) FROM Gost_Klausuren_Kursklausuren), 0) + 1 WHERE ID = NEW.ID;
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Kursklausuren',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Kursklausuren), 0) + 1);
END;


CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Kursklausuren_1 AFTER UPDATE ON Gost_Klausuren_Kursklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Kursklausuren';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Kursklausuren_2 AFTER UPDATE ON Gost_Klausuren_Kursklausuren FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Kursklausuren SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Kursklausuren';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Kursklausuren_3 AFTER UPDATE ON Gost_Klausuren_Kursklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Kursklausuren), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Kursklausuren', coalesce((SELECT max(ID) FROM Gost_Klausuren_Kursklausuren), 0));
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Kursklausuren_4 AFTER UPDATE ON Gost_Klausuren_Kursklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Kursklausuren), 0)
BEGIN
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Kursklausuren_5 AFTER UPDATE ON Gost_Klausuren_Kursklausuren FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kursklausuren') IS NULL
BEGIN
  -- Update der ID in der Tabelle Gost_Klausuren_Kursklausuren erfolgt durch den Autoinkrement-Trigger 2, daher hier auch kein +1, sondern nur den Max-Wert schreiben
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Kursklausuren',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Kursklausuren), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Schuelerklausuren_1 AFTER INSERT ON Gost_Klausuren_Schuelerklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Schuelerklausuren';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Schuelerklausuren_2 AFTER INSERT ON Gost_Klausuren_Schuelerklausuren FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Schuelerklausuren SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Schuelerklausuren';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Schuelerklausuren_3 AFTER INSERT ON Gost_Klausuren_Schuelerklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Schuelerklausuren', coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Schuelerklausuren_4 AFTER INSERT ON Gost_Klausuren_Schuelerklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Schuelerklausuren',  NEW.ID);
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Schuelerklausuren_5 AFTER INSERT ON Gost_Klausuren_Schuelerklausuren FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') IS NULL
BEGIN
  UPDATE Gost_Klausuren_Schuelerklausuren SET ID = coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren), 0) + 1 WHERE ID = NEW.ID;
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Schuelerklausuren',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren), 0) + 1);
END;


CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Schuelerklausuren_1 AFTER UPDATE ON Gost_Klausuren_Schuelerklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Schuelerklausuren';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Schuelerklausuren_2 AFTER UPDATE ON Gost_Klausuren_Schuelerklausuren FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Schuelerklausuren SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Schuelerklausuren';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Schuelerklausuren_3 AFTER UPDATE ON Gost_Klausuren_Schuelerklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Schuelerklausuren', coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren), 0));
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Schuelerklausuren_4 AFTER UPDATE ON Gost_Klausuren_Schuelerklausuren FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren), 0)
BEGIN
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Schuelerklausuren_5 AFTER UPDATE ON Gost_Klausuren_Schuelerklausuren FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren') IS NULL
BEGIN
  -- Update der ID in der Tabelle Gost_Klausuren_Schuelerklausuren erfolgt durch den Autoinkrement-Trigger 2, daher hier auch kein +1, sondern nur den Max-Wert schreiben
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Schuelerklausuren',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Schuelerklausuren_Termine_1 AFTER INSERT ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Schuelerklausuren_Termine';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Schuelerklausuren_Termine_2 AFTER INSERT ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Schuelerklausuren_Termine SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Schuelerklausuren_Termine';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Schuelerklausuren_Termine_3 AFTER INSERT ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren_Termine), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Schuelerklausuren_Termine', coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren_Termine), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Schuelerklausuren_Termine_4 AFTER INSERT ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren_Termine), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Schuelerklausuren_Termine',  NEW.ID);
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Schuelerklausuren_Termine_5 AFTER INSERT ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') IS NULL
BEGIN
  UPDATE Gost_Klausuren_Schuelerklausuren_Termine SET ID = coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren_Termine), 0) + 1 WHERE ID = NEW.ID;
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Schuelerklausuren_Termine',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren_Termine), 0) + 1);
END;


CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Schuelerklausuren_Termine_1 AFTER UPDATE ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Schuelerklausuren_Termine';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Schuelerklausuren_Termine_2 AFTER UPDATE ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Schuelerklausuren_Termine SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Schuelerklausuren_Termine';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Schuelerklausuren_Termine_3 AFTER UPDATE ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren_Termine), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Schuelerklausuren_Termine', coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren_Termine), 0));
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Schuelerklausuren_Termine_4 AFTER UPDATE ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren_Termine), 0)
BEGIN
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Schuelerklausuren_Termine_5 AFTER UPDATE ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine') IS NULL
BEGIN
  -- Update der ID in der Tabelle Gost_Klausuren_Schuelerklausuren_Termine erfolgt durch den Autoinkrement-Trigger 2, daher hier auch kein +1, sondern nur den Max-Wert schreiben
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Schuelerklausuren_Termine',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Schuelerklausuren_Termine), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raeume_1 AFTER INSERT ON Gost_Klausuren_Raeume FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Raeume';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raeume_2 AFTER INSERT ON Gost_Klausuren_Raeume FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Raeume SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Raeume';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raeume_3 AFTER INSERT ON Gost_Klausuren_Raeume FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Raeume), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raeume', coalesce((SELECT max(ID) FROM Gost_Klausuren_Raeume), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raeume_4 AFTER INSERT ON Gost_Klausuren_Raeume FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Raeume), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raeume',  NEW.ID);
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raeume_5 AFTER INSERT ON Gost_Klausuren_Raeume FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') IS NULL
BEGIN
  UPDATE Gost_Klausuren_Raeume SET ID = coalesce((SELECT max(ID) FROM Gost_Klausuren_Raeume), 0) + 1 WHERE ID = NEW.ID;
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raeume',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Raeume), 0) + 1);
END;


CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raeume_1 AFTER UPDATE ON Gost_Klausuren_Raeume FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Raeume';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raeume_2 AFTER UPDATE ON Gost_Klausuren_Raeume FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Raeume SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Raeume';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raeume_3 AFTER UPDATE ON Gost_Klausuren_Raeume FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Raeume), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raeume', coalesce((SELECT max(ID) FROM Gost_Klausuren_Raeume), 0));
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raeume_4 AFTER UPDATE ON Gost_Klausuren_Raeume FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Raeume), 0)
BEGIN
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raeume_5 AFTER UPDATE ON Gost_Klausuren_Raeume FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raeume') IS NULL
BEGIN
  -- Update der ID in der Tabelle Gost_Klausuren_Raeume erfolgt durch den Autoinkrement-Trigger 2, daher hier auch kein +1, sondern nur den Max-Wert schreiben
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raeume',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Raeume), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden_1 AFTER INSERT ON Gost_Klausuren_Raumstunden FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Raumstunden';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden_2 AFTER INSERT ON Gost_Klausuren_Raumstunden FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Raumstunden SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Raumstunden';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden_3 AFTER INSERT ON Gost_Klausuren_Raumstunden FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raumstunden', coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden_4 AFTER INSERT ON Gost_Klausuren_Raumstunden FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raumstunden',  NEW.ID);
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden_5 AFTER INSERT ON Gost_Klausuren_Raumstunden FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') IS NULL
BEGIN
  UPDATE Gost_Klausuren_Raumstunden SET ID = coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden), 0) + 1 WHERE ID = NEW.ID;
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raumstunden',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden), 0) + 1);
END;


CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden_1 AFTER UPDATE ON Gost_Klausuren_Raumstunden FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Raumstunden';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden_2 AFTER UPDATE ON Gost_Klausuren_Raumstunden FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Raumstunden SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Raumstunden';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden_3 AFTER UPDATE ON Gost_Klausuren_Raumstunden FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raumstunden', coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden), 0));
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden_4 AFTER UPDATE ON Gost_Klausuren_Raumstunden FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden), 0)
BEGIN
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden_5 AFTER UPDATE ON Gost_Klausuren_Raumstunden FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden') IS NULL
BEGIN
  -- Update der ID in der Tabelle Gost_Klausuren_Raumstunden erfolgt durch den Autoinkrement-Trigger 2, daher hier auch kein +1, sondern nur den Max-Wert schreiben
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raumstunden',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden_Aufsichten_1 AFTER INSERT ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Raumstunden_Aufsichten';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden_Aufsichten_2 AFTER INSERT ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Raumstunden_Aufsichten SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Raumstunden_Aufsichten';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden_Aufsichten_3 AFTER INSERT ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden_Aufsichten), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raumstunden_Aufsichten', coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden_Aufsichten), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden_Aufsichten_4 AFTER INSERT ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden_Aufsichten), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raumstunden_Aufsichten',  NEW.ID);
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden_Aufsichten_5 AFTER INSERT ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') IS NULL
BEGIN
  UPDATE Gost_Klausuren_Raumstunden_Aufsichten SET ID = coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden_Aufsichten), 0) + 1 WHERE ID = NEW.ID;
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raumstunden_Aufsichten',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden_Aufsichten), 0) + 1);
END;


CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden_Aufsichten_1 AFTER UPDATE ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Raumstunden_Aufsichten';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden_Aufsichten_2 AFTER UPDATE ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Raumstunden_Aufsichten SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Raumstunden_Aufsichten';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden_Aufsichten_3 AFTER UPDATE ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden_Aufsichten), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raumstunden_Aufsichten', coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden_Aufsichten), 0));
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden_Aufsichten_4 AFTER UPDATE ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden_Aufsichten), 0)
BEGIN
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden_Aufsichten_5 AFTER UPDATE ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten') IS NULL
BEGIN
  -- Update der ID in der Tabelle Gost_Klausuren_Raumstunden_Aufsichten erfolgt durch den Autoinkrement-Trigger 2, daher hier auch kein +1, sondern nur den Max-Wert schreiben
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Raumstunden_Aufsichten',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Raumstunden_Aufsichten), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Kalenderinformationen_1 AFTER INSERT ON Gost_Klausuren_Kalenderinformationen FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Kalenderinformationen';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Kalenderinformationen_2 AFTER INSERT ON Gost_Klausuren_Kalenderinformationen FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Kalenderinformationen SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Kalenderinformationen';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Kalenderinformationen_3 AFTER INSERT ON Gost_Klausuren_Kalenderinformationen FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Kalenderinformationen), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Kalenderinformationen', coalesce((SELECT max(ID) FROM Gost_Klausuren_Kalenderinformationen), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Kalenderinformationen_4 AFTER INSERT ON Gost_Klausuren_Kalenderinformationen FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Kalenderinformationen), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Kalenderinformationen',  NEW.ID);
END;


CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Kalenderinformationen_5 AFTER INSERT ON Gost_Klausuren_Kalenderinformationen FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') IS NULL
BEGIN
  UPDATE Gost_Klausuren_Kalenderinformationen SET ID = coalesce((SELECT max(ID) FROM Gost_Klausuren_Kalenderinformationen), 0) + 1 WHERE ID = NEW.ID;
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Kalenderinformationen',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Kalenderinformationen), 0) + 1);
END;


CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Kalenderinformationen_1 AFTER UPDATE ON Gost_Klausuren_Kalenderinformationen FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'Gost_Klausuren_Kalenderinformationen';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Kalenderinformationen_2 AFTER UPDATE ON Gost_Klausuren_Kalenderinformationen FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') IS NOT NULL
BEGIN
  UPDATE Gost_Klausuren_Kalenderinformationen SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'Gost_Klausuren_Kalenderinformationen';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Kalenderinformationen_3 AFTER UPDATE ON Gost_Klausuren_Kalenderinformationen FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM Gost_Klausuren_Kalenderinformationen), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Kalenderinformationen', coalesce((SELECT max(ID) FROM Gost_Klausuren_Kalenderinformationen), 0));
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Kalenderinformationen_4 AFTER UPDATE ON Gost_Klausuren_Kalenderinformationen FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM Gost_Klausuren_Kalenderinformationen), 0)
BEGIN
...
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Kalenderinformationen_5 AFTER UPDATE ON Gost_Klausuren_Kalenderinformationen FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Kalenderinformationen') IS NULL
BEGIN
  -- Update der ID in der Tabelle Gost_Klausuren_Kalenderinformationen erfolgt durch den Autoinkrement-Trigger 2, daher hier auch kein +1, sondern nur den Max-Wert schreiben
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('Gost_Klausuren_Kalenderinformationen',  coalesce((SELECT max(ID) FROM Gost_Klausuren_Kalenderinformationen), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_EigeneSchule_Email_1 AFTER INSERT ON EigeneSchule_Email FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'EigeneSchule_Email';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_EigeneSchule_Email_2 AFTER INSERT ON EigeneSchule_Email FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') IS NOT NULL
BEGIN
  UPDATE EigeneSchule_Email SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'EigeneSchule_Email';
END;


CREATE TRIGGER t_AutoIncrement_INSERT_EigeneSchule_Email_3 AFTER INSERT ON EigeneSchule_Email FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM EigeneSchule_Email), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('EigeneSchule_Email', coalesce((SELECT max(ID) FROM EigeneSchule_Email), 0));
END;


CREATE TRIGGER t_AutoIncrement_INSERT_EigeneSchule_Email_4 AFTER INSERT ON EigeneSchule_Email FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM EigeneSchule_Email), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('EigeneSchule_Email',  NEW.ID);
END;


CREATE TRIGGER t_AutoIncrement_INSERT_EigeneSchule_Email_5 AFTER INSERT ON EigeneSchule_Email FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') IS NULL
BEGIN
  UPDATE EigeneSchule_Email SET ID = coalesce((SELECT max(ID) FROM EigeneSchule_Email), 0) + 1 WHERE ID = NEW.ID;
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('EigeneSchule_Email',  coalesce((SELECT max(ID) FROM EigeneSchule_Email), 0) + 1);
END;


CREATE TRIGGER t_AutoIncrement_UPDATE_EigeneSchule_Email_1 AFTER UPDATE ON EigeneSchule_Email FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') IS NOT NULL AND 
	  NEW.ID > (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email')
BEGIN
  UPDATE Schema_AutoInkremente SET MaxID = NEW.ID WHERE NameTabelle = 'EigeneSchule_Email';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_EigeneSchule_Email_2 AFTER UPDATE ON EigeneSchule_Email FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') IS NOT NULL
BEGIN
  UPDATE EigeneSchule_Email SET ID = (SELECT MaxID FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') + 1 WHERE ID = NEW.ID;
  UPDATE Schema_AutoInkremente SET MaxID = MaxID + 1 WHERE NameTabelle = 'EigeneSchule_Email';
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_EigeneSchule_Email_3 AFTER UPDATE ON EigeneSchule_Email FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') IS NULL AND
	  NEW.ID < coalesce((SELECT max(ID) FROM EigeneSchule_Email), 0)
BEGIN
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('EigeneSchule_Email', coalesce((SELECT max(ID) FROM EigeneSchule_Email), 0));
END;
...
CREATE TRIGGER t_AutoIncrement_UPDATE_EigeneSchule_Email_4 AFTER UPDATE ON EigeneSchule_Email FOR EACH ROW
	WHEN NEW.ID >= 0 AND 
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') IS NULL AND
	  NEW.ID >= coalesce((SELECT max(ID) FROM EigeneSchule_Email), 0)
BEGIN
...
CREATE TRIGGER t_AutoIncrement_UPDATE_EigeneSchule_Email_5 AFTER UPDATE ON EigeneSchule_Email FOR EACH ROW
	WHEN NEW.ID < 0 AND
	  (SELECT max(MaxID) FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email') IS NULL
BEGIN
  -- Update der ID in der Tabelle EigeneSchule_Email erfolgt durch den Autoinkrement-Trigger 2, daher hier auch kein +1, sondern nur den Max-Wert schreiben
  INSERT INTO Schema_AutoInkremente(NameTabelle, MaxID) VALUES ('EigeneSchule_Email',  coalesce((SELECT max(ID) FROM EigeneSchule_Email), 0));
END;



INSERT INTO Schema_Status(Revision) VALUES (18);
