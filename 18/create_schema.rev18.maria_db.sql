...
CREATE TABLE EigeneSchule_Faecher (
  ...
  IstPruefungsordnungsRelevant smallint DEFAULT 1 NOT NULL COMMENT 'Gibt an, ob das Fach für die Prüfungsordnung relevant ist und bei Belegprüfungen genutzt werden soll', 
...
CREATE TABLE Gost_Blockung (
  ...
  Abi_Jahrgang int NOT NULL COMMENT 'Der Abiturjahrgang, dem die Blockung zugeordnet ist', 
  Halbjahr int NOT NULL COMMENT 'Das Halbjahr, welchem die Kursblockung zugeordnet ist (0=EF.1, 1=EF.2, 2=Q1.1, 3=Q1.2, 4=Q2.1, 5=Q2.2)', 
  IstAktiv int DEFAULT 0 NOT NULL COMMENT 'Gibt an, ob die Blockung als aktive Blockung markiert wurde oder nicht: 1 - true, 0 - false.',
...
CREATE TABLE Gost_Blockung_Kurse (
  ID bigint DEFAULT -1 NOT NULL COMMENT 'ID des Kurses in der Blockung (generiert)', 
  Blockung_ID bigint NOT NULL COMMENT 'ID der Blockung', 
  ...
  Kursnummer int NOT NULL COMMENT 'Die Nummer des Kurses in Bezug auf das Fach (Kurse eines Faches sind in einer Blockung üblicherweise von 1 ab durchnummeriert)', 
  IstKoopKurs int DEFAULT 0 NOT NULL COMMENT 'Gibt an, ob es sich um einen Kooperations-Kurs mit einer anderen Schule handelt oder nicht: 1 - true, 0 - false ', 
  BezeichnungSuffix varchar(255) COMMENT 'Ein Suffix, welches der Kursbezeichnung ggf. angehangen wird (kann z.B. zum Kennzeichnen von Kooperationskursen verwendet werden)', 
  Schienenanzahl int DEFAULT 1 NOT NULL COMMENT 'Gibt die Anzahl der Schienen an, die für den Kurs in der Blockung verwendet werden soll (normalerweise 1)', 
  Wochenstunden int DEFAULT 3 NOT NULL COMMENT 'Die Anzahl der Wochenstunden für den Kurs',
...
CREATE TABLE Gost_Blockung_Regeln (
  ID bigint DEFAULT -1 NOT NULL COMMENT 'ID der Regel (generiert)', 
  Blockung_ID bigint NOT NULL COMMENT 'ID der Blockung', 
  Typ int NOT NULL COMMENT 'Die ID des Typs der Regeldefinition (siehe Core-Type GostKursblockungRegeltyp)',
...
CREATE TABLE Gost_Blockung_Schienen (
  ID bigint DEFAULT -1 NOT NULL COMMENT 'ID der Schiene in der Blockung (generiert)', 
  Blockung_ID bigint NOT NULL COMMENT 'ID der Blockung', 
  Nummer int NOT NULL COMMENT 'Die Nummer der Schiene, beginnend bei 1', 
  Bezeichnung varchar(255) DEFAULT 'Schiene' NOT NULL COMMENT 'Bezeichnung der Schiene (z.B. LK Schiene 1)', 
  Wochenstunden int DEFAULT 3 NOT NULL COMMENT 'Die Anzahl der Wochenstunden, welche der Schiene zugeordnet sind',
...
CREATE TABLE Gost_Blockung_Zwischenergebnisse (
  ID bigint DEFAULT -1 NOT NULL COMMENT 'ID der Zwischenergebnisses einer Blockung (generiert)', 
  Blockung_ID bigint NOT NULL COMMENT 'ID der Blockung', 
  IstAktiv int DEFAULT 0 NOT NULL COMMENT 'Gibt an, ob das Zwischenergebnis als aktives Zwischenergebnis einer Blockung markiert wurde oder nicht: 1 - true, 0 - false ',
...
CREATE TABLE Gost_Blockung_Regelparameter (
  Regel_ID bigint NOT NULL COMMENT 'ID des Regel-Parameters', 
  Nummer int NOT NULL COMMENT 'Die Nummer des Parameters der Regel, beginnend bei 1', 
  Parameter bigint NOT NULL COMMENT 'Der Wert des Parameters der Regel (hängt vom Typ der Regel ab)',
...
CREATE TABLE Gost_Blockung_Zwischenergebnisse_Kurs_Schienen (
  Zwischenergebnis_ID bigint NOT NULL COMMENT 'Kurs-Schienen-Zuordnung eines Zwischenergebnisses: ID des Zwischenergebnisses', 
  Blockung_Kurs_ID bigint NOT NULL COMMENT 'Kurs-Schienen-Zuordnung eines Zwischenergebnisses: ID des Kurses', 
  Schienen_ID bigint NOT NULL COMMENT 'Kurs-Schienen-Zuordnung eines Zwischenergebnisses: ID der Schiene aus der Blockung',
...
CREATE TABLE Gost_Blockung_Kurslehrer (
  Blockung_Kurs_ID bigint NOT NULL COMMENT 'ID des Kurses', 
  Lehrer_ID bigint NOT NULL COMMENT 'ID des Lehrers, welcher dem Kurs zugeordnet ist', 
  Reihenfolge int DEFAULT 1 NOT NULL COMMENT 'Eine Reihenfolge für die Lehrer, z.B. zur Unterscheidung des eigentlichen Kurslehrer (z.B. 1) und einer Zusatzkraft (z.B. 2)', 
  Wochenstunden int DEFAULT 3 NOT NULL COMMENT 'Die Anzahl der Wochenstunden für die der Lehrer in dem Kurs eingesetzt wird',
...
CREATE TABLE LehrerNotenmodulCredentials (
  Lehrer_ID bigint NOT NULL COMMENT 'Die LehrerID des Lehrers, für den die Credentials gelten', 
  Initialkennwort varchar(255) NOT NULL COMMENT 'Initialkennwort für den Credential-Datensatz', 
  PasswordHash varchar(255) NOT NULL COMMENT 'Passwordhash für den Credential-Datensatz',
...
CREATE TABLE Gost_Blockung_Zwischenergebnisse_Kurs_Schueler (
  Zwischenergebnis_ID bigint NOT NULL COMMENT 'Kurs-Schüler-Zuordnung eines Zwischenergebnisses: ID des Zwischenergebnisses', 
  Blockung_Kurs_ID bigint NOT NULL COMMENT 'Kurs-Schüler-Zuordnung eines Zwischenergebnisses: ID des Kurses', 
  Schueler_ID bigint NOT NULL COMMENT 'Kurs-Schüler-Zuordnung eines Zwischenergebnisses: ID des Schülers',
...
CREATE TABLE DavRessourceCollections (
  Benutzer_ID bigint NOT NULL COMMENT 'Die ID des Benutzers, zu dem der Datensatz gehört', 
  ID bigint DEFAULT -1 NOT NULL COMMENT 'ID der WebDav-Ressourcensammlung', 
  Typ int NOT NULL COMMENT 'Gibt den Typ dieser Sammlung an, bspw Adressbuch oder Kalender', 
  Anzeigename varchar(120) NOT NULL COMMENT 'Der Anzeigename der Ressourcensammlung', 
  Beschreibung varchar(2047) COMMENT 'Die Beschreibung der Ressourcensammlung', 
  SyncToken datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Das SyncToken der Ressourcensammlung', 
  geloeschtam datetime(3) COMMENT 'Der Zeitpunkt, an dem diese ggf. Ressource gelöscht wurde.',
...
CREATE TABLE DavRessourceCollectionsACL (
  ID bigint DEFAULT -1 NOT NULL COMMENT 'ID des ACL Eintrags', 
  Benutzer_ID bigint NOT NULL COMMENT 'Die ID des Benutzers dieses ACL-Eintrags', 
  RessourceCollection_ID bigint NOT NULL COMMENT 'Die ID der RessourceCollection dieses ACL-Eintrags', 
  berechtigungen varchar(20) NOT NULL COMMENT 'Gibt die Berechtigungen dieses ACL-Eintrags wieder, ähnlich einer unix-file permission - bspw. ''r-'' für nur Leserecht oder ''rw'' für Lese- und Schreibrecht.',
...
CREATE TABLE DavRessources (
  ID bigint DEFAULT -1 NOT NULL COMMENT 'ID der Dav Ressource', 
  DavRessourceCollection_ID bigint NOT NULL COMMENT 'Id der Ressourcensammlung, deren Teil diese Ressource ist', 
  UID varchar(120) NOT NULL COMMENT 'Die UID der Ressource', 
  lastModified datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Das Datum an dem die Ressource zuletzt geändert wurde, als Synctoken einsetzbar', 
  KalenderTyp varchar(120) NOT NULL COMMENT 'Die Art der Kalenderressource, wenn es eine Kalenderressource ist', 
  KalenderStart datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Start der Kalenderressource, wenn es eine Kalenderressource ist', 
  KalenderEnde datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Das Ende der Kalenderressource, wenn es eines Kalenderressource ist', 
  ressource longblob NOT NULL COMMENT 'Die Daten der Ressource', 
  geloeschtam datetime(3) COMMENT 'Der Zeitpunkt, an dem diese ggf. Ressource gelöscht wurde.',
...
CREATE TABLE DavSyncTokenSchueler (
  ID bigint DEFAULT -1 NOT NULL COMMENT 'ID des Schülers', 
  SyncToken datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an für einen Card-DAV-Eintrag relevanten Schülerdaten.',
...
CREATE TABLE DavSyncTokenLehrer (
  ID bigint DEFAULT -1 NOT NULL COMMENT 'ID des Lehrers', 
  SyncToken datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an für einen Card-DAV-Eintrag relevanten Lehrerdaten.',
...
CREATE TABLE EnmLeistungsdaten (
  ID bigint NOT NULL COMMENT 'ID der Leistungsdaten', 
  tsNotenKrz datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an der Note.', 
  tsNotenKrzQuartal datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an der Note.', 
  tsFehlStd datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an den Fehlstunden.', 
  tsuFehlStd datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an den unentschuldigten Fehlstunden.', 
  tsLernentw datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an den fachbezogenen Bemerkungen.', 
  tsWarnung datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung, ob gemahnt wird.',
...
CREATE TABLE EnmLernabschnittsdaten (
  ID bigint NOT NULL COMMENT 'ID der Lernabschnittsdaten', 
  tsSumFehlStd datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an der Summe der Fehlstunden.', 
  tsSumFehlStdU datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an der Summe der unentschuldigten Fehlstunden.', 
  tsZeugnisBem datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an den Zeugnisbemerkungen.', 
  tsASV datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an den Bemerkungen zum Arbeits- und Sozialverhalten.', 
  tsAUE datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an den Bemerkungen zum außerunterrichtlichen Engagement.', 
  tsBemerkungVersetzung datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an den Bemerkungen zur Versetzung.',
...
CREATE TABLE EnmTeilleistungen (
  ID bigint NOT NULL COMMENT 'ID der Teilleistungen', 
  tsDatum datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an dem Datum der Teilleistung, wann diese erbracht wurde.', 
  tsLehrer_ID datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an der Lehrer-ID.', 
  tsArt_ID datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung der Teilleistungsart.', 
  tsBemerkung datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an der Bemerkung.', 
  tsNotenKrz datetime(3) DEFAULT (CURRENT_TIME()) NOT NULL COMMENT 'Der Zeitstempel der letzten Änderung an der Note.',
...
CREATE TABLE Gost_Klausuren_Termine (
  ...
  IstHaupttermin int DEFAULT 1 NOT NULL COMMENT 'Gibt an, ob es sich bei dem Termin um den Haupttermin (1) handelt oder einen Nachschreibtermin (0).', 
  NachschreiberZugelassen int DEFAULT 0 NOT NULL COMMENT 'Gibt an, ob bei einem Haupttermin Nachschreibklausuren zugelassen sind (1) oder nicht (0).',
...
CREATE TABLE Gost_Klausuren_Termine_Jahrgaenge (
  Termin_ID bigint NOT NULL COMMENT 'Termin_ID des Klausurtermins', 
  Abi_Jahrgang int NOT NULL COMMENT 'Der Abiturjahrgang, der zum Klausurtermin zugelassen werden soll.', 
  Quartal int NOT NULL COMMENT 'Das Quartal, in dem die Klausur geschrieben wird.', 
  Bezeichnung longtext COMMENT 'Text für Bezeichnung des Klausurtermins', 
  Bemerkungen longtext COMMENT 'Text für Bemerkungen des Klausurtermins', 
  IstHaupttermin int DEFAULT 1 NOT NULL COMMENT 'Gibt an, ob es sich bei dem Termin um den Haupttermin (1) handelt oder einen Nachschreibtermin (0).', 
...
CREATE TABLE Gost_Klausuren_Kursklausuren (
  ...
  Bemerkungen longtext COMMENT 'Text für Bemerkungen zur Kursklausur',
...
CREATE TABLE Gost_Klausuren_Schuelerklausuren (
  ...
  Bemerkungen longtext COMMENT 'Text für Bemerkungen zur Schuelerklausur',
...
CREATE TABLE Gost_Klausuren_Schuelerklausuren_Termine (
  ID bigint DEFAULT -1 NOT NULL COMMENT 'ID des Schülerklausur-Termins (generiert)', 
  Schuelerklausur_ID bigint NOT NULL COMMENT 'ID der Schülerklausur', 
  Folge_Nr smallint DEFAULT 0 NOT NULL COMMENT 'Folgenummer des Schülerklausur-Termins', 
  Termin_ID bigint COMMENT 'ID des Klausurtermins, null falls Termin der Kursklausur', 
  Startzeit time COMMENT 'Startzeit der Klausur, wenn abweichend von Startzeit des Klausurtermins', 
  Bemerkungen longtext COMMENT 'Text für Bemerkungen des Schuelerklausurtermins',
...
CREATE TABLE Gost_Klausuren_Raumstunden (
  ID bigint DEFAULT -1 NOT NULL COMMENT 'ID der Raumstunde (generiert)', 
...
CREATE TABLE Gost_Klausuren_SchuelerklausurenTermine_Raumstunden (
  Schuelerklausurtermin_ID bigint NOT NULL COMMENT 'ID des Schuelerklausurtermins', 
  Raumstunde_ID bigint NOT NULL COMMENT 'ID der Klausurraumstunde',
...
CREATE TABLE Gost_Klausuren_Raumstunden_Aufsichten (
  ...
  Raumstunde_ID bigint NOT NULL COMMENT 'ID der Klausur-Raumstunde', 
...
CREATE TABLE EigeneSchule_Email (
  ID bigint DEFAULT -1 NOT NULL COMMENT 'Die ID des Eintrags', 
  Domain varchar(255) COMMENT 'Die Default Email-Domain der Schule', 
  SMTPServer varchar(255) NOT NULL COMMENT 'Die Adresse des SMTP-Servers ', 
  SMTPPort int DEFAULT 25 NOT NULL COMMENT 'Die Port-Adresse des SMTP-Servers', 
  SMTPStartTLS int DEFAULT 1 NOT NULL COMMENT 'Gibt an, ob StartTLS für die SMTP-Verbindung genutzt wird (1) oder nicht (0)', 
  SMTPUseTLS int DEFAULT 0 NOT NULL COMMENT 'Gibt an, ob TLS für die SMTP-Verbindung genutzt wird (1) oder nicht (0). Ist TLS gesetzt, so wird entweder ein Zertifikat im Key-Store des Servers benötigt oder es muss einem Host vertraut werden (siehe Spalte SMTPTrustTLSHost)', 
  SMTPTrustTLSHost varchar(255) COMMENT 'Gibt an, falls gesetzt, welchem Host - unabhängig von Zertifikaten - vertraut werden kann, ''*'' für jeden beliebigen Host',
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Blockung
BEFORE INSERT
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Blockung
BEFORE UPDATE
  ON Gost_Blockung FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Blockung_Kurse
BEFORE INSERT
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Blockung_Kurse
BEFORE UPDATE
  ON Gost_Blockung_Kurse FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Blockung_Regeln
BEFORE INSERT
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Blockung_Regeln
BEFORE UPDATE
  ON Gost_Blockung_Regeln FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Blockung_Schienen
BEFORE INSERT
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Blockung_Schienen
BEFORE UPDATE
  ON Gost_Blockung_Schienen FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Blockung_Zwischenergebnisse
BEFORE INSERT
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Blockung_Zwischenergebnisse
BEFORE UPDATE
  ON Gost_Blockung_Zwischenergebnisse FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_DavRessourceCollections
BEFORE INSERT
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_DavRessourceCollections
BEFORE UPDATE
  ON DavRessourceCollections FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_DavRessourceCollectionsACL
BEFORE INSERT
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_DavRessourceCollectionsACL
BEFORE UPDATE
  ON DavRessourceCollectionsACL FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_DavRessources
BEFORE INSERT
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_DavRessources
BEFORE UPDATE
  ON DavRessources FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_DavSyncTokenSchueler
BEFORE UPDATE
  ON DavSyncTokenSchueler FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_DavSyncTokenLehrer
BEFORE INSERT
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_DavSyncTokenLehrer
BEFORE UPDATE
  ON DavSyncTokenLehrer FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_INSERT_EnmTeilleistungen AFTER INSERT ON SchuelerEinzelleistungen FOR EACH ROW
...
delimiter $
CREATE TRIGGER t_UPDATE_EnmTeilleistungen AFTER UPDATE ON SchuelerEinzelleistungen FOR EACH ROW
BEGIN
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Schuelerklausuren_Termine
BEFORE INSERT
  ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
  SELECT MaxID INTO tmpID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Schuelerklausuren_Termine';
  IF tmpID IS NULL THEN
    SELECT max(ID) INTO tmpID FROM Gost_Klausuren_Schuelerklausuren_Termine;
    IF tmpID IS NULL THEN
      SET tmpID = 0;
    END IF;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Schuelerklausuren_Termine
BEFORE UPDATE
  ON Gost_Klausuren_Schuelerklausuren_Termine FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden
BEFORE INSERT
  ON Gost_Klausuren_Raumstunden FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
  SELECT MaxID INTO tmpID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden';
  IF tmpID IS NULL THEN
    SELECT max(ID) INTO tmpID FROM Gost_Klausuren_Raumstunden;
    IF tmpID IS NULL THEN
      SET tmpID = 0;
    END IF;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden
BEFORE UPDATE
  ON Gost_Klausuren_Raumstunden FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_Gost_Klausuren_Raumstunden_Aufsichten
BEFORE INSERT
  ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
  SELECT MaxID INTO tmpID FROM Schema_AutoInkremente WHERE NameTabelle='Gost_Klausuren_Raumstunden_Aufsichten';
  IF tmpID IS NULL THEN
    SELECT max(ID) INTO tmpID FROM Gost_Klausuren_Raumstunden_Aufsichten;
    IF tmpID IS NULL THEN
      SET tmpID = 0;
    END IF;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_Gost_Klausuren_Raumstunden_Aufsichten
BEFORE UPDATE
  ON Gost_Klausuren_Raumstunden_Aufsichten FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_INSERT_EigeneSchule_Email
BEFORE INSERT
  ON EigeneSchule_Email FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
  SELECT MaxID INTO tmpID FROM Schema_AutoInkremente WHERE NameTabelle='EigeneSchule_Email';
  IF tmpID IS NULL THEN
    SELECT max(ID) INTO tmpID FROM EigeneSchule_Email;
    IF tmpID IS NULL THEN
      SET tmpID = 0;
    END IF;
...
delimiter $
CREATE TRIGGER t_AutoIncrement_UPDATE_EigeneSchule_Email
BEFORE UPDATE
  ON EigeneSchule_Email FOR EACH ROW
BEGIN
  DECLARE tmpID bigint;
